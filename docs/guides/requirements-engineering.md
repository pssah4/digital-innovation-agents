...
The skill ends with the standard three-part [Handoff Ritual](../concepts/handoff-rituals): artifact report, phase-end commit (`chore(re): {ITEM-ID} RE complete` with the `DIA-Phase: re-done` and `DIA-Handoff: {ITEM-ID} -> architecture` trailers; open questions and assumptions in the commit body) plus `tag-phase --phase re` and `sync-status --item {ITEM-ID}`, transition question. When the EPIC ID is freshly assigned, the handoff also runs `flow.py promote-to-epic --item EPIC-NN --rename-branch` to rewrite the parent issue, create sub-issues, and rename the feature branch. All flow.py calls are no-ops outside `mode = "github-sync"`. The next phase is `/architecture`.
