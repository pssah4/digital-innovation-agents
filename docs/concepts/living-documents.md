...
| `_devprocess/context/BACKLOG.md` | BL-NNN rows (Planned/Active/Review/Done/Waiting/Deferred), dashboard counts, new findings, status transitions, commit SHAs; binding format in `BACKLOG-TEMPLATE.md` |
| `_devprocess/context/20_bugs.md` | Every bug found, with FIX-NN ID, causal chain, commit SHA |
| `_devprocess/context/METRICS.md` | Cycle time per FEATURE, drift count, hypothesis status, phase transitions, cross-phase trigger counts. Append-additive, written from inside existing phase actions |
