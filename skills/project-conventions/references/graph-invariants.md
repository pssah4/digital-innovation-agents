...
| E-15 | In Epic-Detail-Dateien (`_devprocess/requirements/epics/EPIC-*.md`) listet die `## Features`- oder `## MVP Features`-Tabelle Sub-Items mit einer Status-Spalte. Diese Status-Werte muessen mit der Status-Spalte der korrespondierenden BACKLOG-Row kompatibel sein (Done/Released/Implementiert auf einer Seite vs. Backlog/Ready/In Progress/Planned/Geplant/Not Started/Open/Draft auf der anderen). | Pro Epic-Detail-Datei: Features-Tabelle parsen, ID und letzte Spalte (Status) extrahieren, mit Backlog-Index abgleichen. Inkompatible Paare melden als `status-drift-detail-vs-backlog`. Severity medium. |
