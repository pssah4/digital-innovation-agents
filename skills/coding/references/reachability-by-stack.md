...
This file operationalises the universal Done-Definition contract from
`/coding` Phase 4a Verification Gate (steps 6 and 7) and the
`/requirements-engineering` FEATURE subtype model. The contract is
universal; the tooling that verifies it is per-language. Each entry
below carries:
...
3. Open a PR titled `feat(coding-ref): add reachability profile for
   <stack>`.
4. If your stack needs new activation-path types not in the existing
   set, add them to the FEATURE template in
   `skills/requirements-engineering/templates/FEATURE-TEMPLATE.md`
   so users can pick them in their FEATURE specs.
