...
`promote-to-epic` round-trip plus two new flags for retroactive
sync of an existing backlog into GitHub Issues.

### Fixed

- `flow.py promote-to-epic` now creates real GitHub sub-issue
  parent-child relations via the REST `sub_issues` endpoint instead
  of only writing a markdown checklist into the epic body. Existing
  flat sub-issues from earlier sync runs are linked retroactively
  on the next promote-to-epic call (idempotent).
- The issue body for an item now mirrors the artefact's detail file
  under `_devprocess/requirements/{epics,features,fixes,improvements}/`
  rather than just the backlog title. The hardcoded V-Model phase
  checklist and the `Backlog item / Type / Epic / Priority` prelude
  are removed; status, phase, and priority live in the BACKLOG row
  and on GitHub via labels and the project board, not in the body.

### Added

- New flag `flow.py create-issue --update-body`. When the issue
  already exists, overwrite its body with the current detail-file
  content. Used for retro-sync of legacy projects.
- New flag `flow.py promote-to-epic --sync-bodies`. Refreshes the
  epic body and every sub-issue body from their detail files, in
  addition to creating sub-issue links. Default behaviour is
  unchanged: without the flag, existing bodies stay untouched.
- New helpers in `flow.py`: `detail_file_dir`, `find_detail_file`,
  `read_detail_body`, `gh_repo_slug`, `issue_database_id`,
  `link_sub_issue`. The link helper resolves issue numbers to their
  REST database id, which the GitHub sub-issues endpoint requires.

### Changed

- Issue bodies generated by `flow.py create-issue` now begin with a
  one-line backlog pointer block, followed by the detail-file body
  (frontmatter and template HTML comments stripped). Bodies for items
  without a detail file degrade to a single backlog pointer plus an
  explicit `_No detail file ... yet._` notice.

### Not in this release

- No automatic bulk `sync-backlog` subcommand yet. Retro-sync today
  runs per epic via `promote-to-epic --sync-bodies`. A bulk wrapper
  is queued for the next release.
- The legacy `## Sub-Issues` markdown checklist in the epic body
  remains for backward compatibility. It will be removed once the
  native sub-issue relation is the only source.
